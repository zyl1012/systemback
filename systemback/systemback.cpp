...
void systemback::unitimer()
{
    if(! utblck)
    {
        ...
        else if(! sstart)
        {
            ...
            {
                if(ui->installmenu->isEnabled() && ui->fullnamepipe->isVisible() && ui->passwordpipe->isVisible() && (ui->rootpassword1->text().isEmpty() || ui->rootpasswordpipe->isVisible()) && ! ui->installnext->isEnabled())
                    ui->installnext->setEnabled(true);
            }
...
void systemback::systemcopy()
{
...

    {
        ...
        for(cQStr &vals : msort)
        {
            ...
            if(fstype.length() > 2)
            {
                QStr lbl("RG@" % (mpoint.startsWith('/') ? sb::right(mpoint, -1) : mpoint));
    ...
    // RGBux mod
    sb::copy("/etc/RGBux/system.conf", "/.sbsystemcopy/etc/RGBux/system.conf");
    sb::copy("/etc/default/grub", "/.sbsystemcopy/etc/default/grub");
